...
>
> See **[TypeScript-ESLint Rule Documentation](https://typescript-eslint.io/rules/consistent-type-assertions)** for documentation.
...
:::note
Type assertions are also commonly referred to as "type casting" in TypeScript.
However, that term is technically slightly different from what is understood by type casting in other languages.
...
The compiler will warn for excess properties with this syntax but not for missing _required_ fields. For example: `const x: { foo: number } = {};` will fail to compile, but `const x = {} as { foo: number }` will succeed.
